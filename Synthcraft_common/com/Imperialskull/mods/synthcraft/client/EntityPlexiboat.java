...
public class EntityPlexiboat extends Entity {
    ...
    public EntityPlexiboat(World world) {
      super(world);
...
*/
    ...
    public EntityPlexiboat(World world, double d, double d1, double d2) {
      this(world);
    ...
    public boolean attackEntityFrom(DamageSource damagesource, int i) {
      ...
      if (getDamageTaken() > 40) {
        dropItemWithOffset(Synthcraft.BlockGlowingplexiglass.blockID, 1, 0.0F);
        setDead();
      }
    ...
    public void onUpdate() {
      ...
      if (!isCollidedHorizontally && d1 <= 0.14999999999999999D) {
        motionX *= 0.99000000953674316D;
        motionY *= 0.94999998807907104D;
        motionZ *= 0.99000000953674316D;
      }
      ...
      if (list != null && list.size() > 0) {
        for (int j1=0; j1<list.size(); j1++) {
          ...
          if (entity != riddenByEntity && entity.canBePushed() && (entity instanceof EntityPlexiboat))
      ...
      for (int k1=0; k1<4; k1++) {
        ...
        if (worldObj.getBlockId(l1, i2, j2) == Block.snow.blockID)
          worldObj.setBlockMetadataWithNotify(l1, i2, j2, 0,0);
      }
